...
static void gdt_make_entry(gdt_entry_t * gate,
...
void gdt_init(cpu_t * cpuinfo)
{
    ...
    gdt_make_entry(&(gdt->kdata16), 0, 0xFFFF, AC_RW | AC_PR | AC_ST);
    ...
    gdt_make_entry(&(gdt->kdata32), 0, 0xFFFFFFFF, AC_RW | AC_PR | AC_ST);
    ...
    gdt_register_t g = {.offset = (uint64_t) gdt,.size =
            sizeof(gdt_table_t) - 1
    };
