...
static void klog_putbin(klog_info_t *k, uint64_t n, int width, bool mid_blank)
{
    int cnt = 0;
    for (int i = 63; i >= 0; i--) {
        cnt++;
        ...
        if ((i % 4 == 0) && i > 0 && mid_blank) klog_putch(k, ' ');
    }
    klog_putch(k, 'b'); 
}

static void klog_putint(klog_info_t *k, int64_t n, int width, bool zero_filling)
{
    int64_t n_val = n;
    ...
    for (uint64_t i = 0; s[i] != '\0'; i++) {
        ...
        case '%': {
            ...
            case '%':
                ...
                break;
            case 'd':
                klog_putint(k, va_arg(args, int64_t), arg_width, zero_filling);
                break;
            case 'x':
                klog_puthex(k, va_arg(args, uint64_t), arg_width);
                break;
            case 'b':
                klog_putbin(k, va_arg(args, uint64_t), arg_width, !zero_filling);
                break;
            case 's':
...
void klog_vprintf(klog_level_t level, const char *s, ...)
{
    ...
    if (level < KLOG_LEVEL_UNK) {
        ...
        klog_vprintf_wrapper(&logout, "%04d-%02d-%02d %02d:%02d:%02d %03d ",
