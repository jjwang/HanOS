/**-----------------------------------------------------------------------------
 ...
 @verbatim
   ...
   These helpers validate that a user pointer range belongs to a process and is
...
/* True when [uptr, uptr+len) is fully mapped and user-accessible in process t. */
bool user_range_ok(process_t * t, const void *uptr, uint64_t len);

/* Copy to/from the *current* process. Return the number of bytes NOT copied
...
/* Copy a NUL-terminated string from the current process. Returns the length on
