...
static ata_device_t ata_primary_master = {.io_base = 0x1F0,.control =
        0x3F6,.slave = 0
};

static ata_device_t ata_primary_slave = {.io_base = 0x1F0,.control =
        0x3F6,.slave = 1
};

static ata_device_t ata_secondary_master = {.io_base = 0x170,.control =
        0x376,.slave = 0
};

static ata_device_t ata_secondary_slave = {.io_base = 0x170,.control =
        0x376,.slave = 1
};
...
static int ata_device_detect(ata_device_t * dev)
{
    ...
    if ((cl == 0x00 && ch == 0x00) || (cl == 0x3C && ch == 0xC3)) {
        ...
        vfs_tnode_t *tnode =
            vfs_path_to_node(devname, CREATE, VFS_NODE_BLOCK_DEVICE);
        vfs_inode_t *inode =
                    ...
                    uint8_t * target)
{
    ...
    port_outb(bus + ATA_REG_HDDEVSEL,
    ...
    if (mbr.signature[0] == 0x55 && mbr.signature[1] == 0xAA) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (mbr.partitions[i].type == 0x0B
                ...
                strncat(partition_path, partition_name,
