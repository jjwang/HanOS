...
int fat32_read_entry(vfs_inode_t * this, uint32_t cluster, uint64_t index,
                     fat32_entry_t * dest)
{
...
int fat32_write_entry(vfs_inode_t * this, fat32_entry_t * src)
{
    ...
    uint64_t index = src->dir_entry_index;
                   ...
                   void *buff)
{
    ...
    uint64_t sector_num =
        DIV_ROUNDUP(offset + len, id->bs.bytes_per_sector);
    uint8_t *dd =
        (uint8_t *) kmalloc(sector_num * id->bs.bytes_per_sector);
    ...
    uint64_t retlen =
        MIN(sector_num * id->bs.bytes_per_sector - offset, len);
                    ...
                    const void *buff)
{
    ...
    while (true) {
        ...
        temp_cluster =
            fat32_get_next_cluster(cluster, id->fat, id->fat_len);
        if (temp_cluster == 0) {
...
int64_t fat32_getdent(vfs_inode_t * this, uint64_t pos,
                      vfs_dirent_t * dirent)
{
    uint64_t num = 0;

    for (uint64_t i = 0; i < vec_length(&fat32.filelist); i++) {
...
int64_t fat32_refresh(vfs_inode_t * this)
{
    ...
    uint64_t cluster_len =
        id->bs.sectors_per_cluster * id->bs.bytes_per_sector;
    ...
    while (true) {
        ...
        for (uint64_t i = 0; i < temp_len / sizeof(fat_dir_entry_t);) {
            fat_dir_entry_t *fe, *fe1;

            fe = (fat_dir_entry_t *) (temp_buffer +
            ...
            fe1 =
                (fat_dir_entry_t *) (temp_buffer + temp_len -
            ...
            if (fe->attribute & FAT32_ATTR_LONGNAME) {
                ...
                uint64_t count = fat32_get_long_filename((fat_lfn_entry_t
                                                          *) fe,
                                                         (fat_lfn_entry_t
                                                          *) fe1, fn);
                if (count > 0) {
                    ...
                    uint8_t *buf =
                        (uint8_t *) kmalloc(temp_len + cluster_len -
                                            i * sizeof(fat_dir_entry_t));
            ...
            fe = (fat_dir_entry_t *) (temp_buffer +
                                      i * sizeof(fat_dir_entry_t));
            if (strlen(fn) == 0) {
                fat32_get_short_filename((char *) fe->file_name_and_ext,
                                         fn, sizeof(fn));
            }
...
void fat32_dump_entry(fat32_entry_t fe)
{
    ...
    if (fe.attribute & FAT32_ATTR_READ_ONLY)
        klogu("  Attribute    : readonly\n");
    if (fe.attribute & FAT32_ATTR_HIDDEN)
        klogu("  Attribute    : hidden\n");
    if (fe.attribute & FAT32_ATTR_SYSTEM)
        klogu("  Attribute    : system\n");
    if (fe.attribute & FAT32_ATTR_VOLUME_ID)
        klogu("  Attribute    : volumeid\n");
    if (fe.attribute & FAT32_ATTR_DIRECTORY)
        klogu("  Attribute    : directory\n");
    if (fe.attribute & FAT32_ATTR_ARCHIVE)
        klogu("  Attribute    : archive\n");
    if (fe.attribute & FAT32_ATTR_LONGNAME)
...
fat32_entry_t fat32_parse_path(vfs_inode_t * this, const char *path)
{
    ...
    for (i = 0;; i++) {
        ...
        if (i == id->bs.sectors_per_cluster * 16) {
            ...
            cluster =
                fat32_get_next_cluster(cluster, id->fat, id->fat_len);
            if (cluster * id->bs.sectors_per_cluster + 16 >=
                id->bs.total_sectors)
                break;
        }
...
{
    uint64_t last_idx = 0, cur_idx = 0, dir_count = 0;
    ...
    if (fe.attribute != 0 && fe.cluster_begin != 0) {
        ...
        uint64_t i, pathlen = strlen(path);
...
{
    vfs_inode_t *ret =
        vfs_alloc_inode(VFS_NODE_MOUNTPOINT, 0777, 0, &fat32, NULL);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (mbr.partitions[i].type == 0x0B
                ...
                if (fat_boot->table_size_16 != 0
                    || fat_boot->total_sectors_16 != 0) {
                    kpanic
                        ("FAT32 file system contains FAT16 parameters\n");
                }
                char vol_name[12] = { 0 };
                memcpy(vol_name,
