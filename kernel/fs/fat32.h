...
static inline uint32_t fat32_get_next_cluster(uint32_t cluster,
                                              uint32_t * fat,
                                              uint32_t fat_len)
{
    if (cluster >= fat_len / 4)
        return 0;
    if (fat[cluster] >= 0xFFFFFFF8)
        return 0;
...
static inline uint32_t fat32_get_free_cluster(uint32_t * fat,
                                              uint32_t fat_len)
{
...
static inline void fat32_get_short_filename(char *file_name_and_ext,
                                            char *fname,
                                            uint32_t fname_size)
{
    int64_t i;
...
static inline uint32_t fat32_get_long_filename(fat_lfn_entry_t * lfne,
                                               fat_lfn_entry_t * lfne_last,
                                               char *fname)
{
    ...
    while (true) {
        ...
        if (first)
            entry_num = idx;
        if (idx == 1 && count == entry_num)
            return count;

        if ((uint64_t) lfne == (uint64_t) lfne_last)
            break;
        lfne =
            (fat_lfn_entry_t *) ((uint64_t) lfne +
                                 sizeof(fat_lfn_entry_t));
    }
